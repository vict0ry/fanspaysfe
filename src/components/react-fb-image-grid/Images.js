...
export default class Images extends Component {
  static defaultProps = {
    images: [],
    hideOverlay: false,
    renderOverlay: () => t('COMMON.PREVIEW_IMAGE'),
  ...
  renderTwo() {
    ...
    return <Grid>
      <Grid container>
        <Grid item xs={6} md={6} className="border height-two background"
              onClick={this.openModal.bind(this, conditionalRender ? 1 : 0)}
              style={{ background: `url(${conditionalRender ? images[1] : images[0]})`, position: 'relative' }}>
          {this.renderOverlay()}
        </Grid>
        <Grid item xs={6} md={6} className="border height-two background"
              onClick={this.openModal.bind(this, conditionalRender ? 2 : 1)}
              style={{ background: `url(${conditionalRender ? images[2] : images[1]})`, position: 'relative' }}>
          {overlay}
        </Grid>
      </Grid>
    </Grid>
  ...
  renderThree(more) {
    ...
    return <Grid>
      <Grid container>
        <Grid item xs={6} md={4} className="border height-three background"
              onClick={this.openModal.bind(this, conditionalRender ? 1 : 2)}
              style={{ background: `url(${conditionalRender ? images[1] : images[2]})`, position: 'relative' }}>
          {this.renderOverlay(conditionalRender ? 1 : 2)}
        </Grid>
        <Grid item xs={6} md={4} className="border height-three background"
              onClick={this.openModal.bind(this, conditionalRender ? 2 : 3)}
              style={{ background: `url(${conditionalRender ? images[2] : images[3]})`, position: 'relative' }}>
          {this.renderOverlay(conditionalRender ? 2 : 3)}
        </Grid>
        <Grid item xs={6} md={4} className="border height-three background"
              onClick={this.openModal.bind(this, conditionalRender ? 3 : 4)}
              style={{ background: `url(${conditionalRender ? images[3] : images[4]})`, position: 'relative' }}>
          {overlay}
        </Grid>
      </Grid>
    </Grid>
